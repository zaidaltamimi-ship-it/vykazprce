...
function Workspace({ authUser, profile, users }) {
  ...
  return (
    <div style={S.app}>
      ...
      >
        {tab === "vykazy" && <TabVykazy data={data} actions={actions} showToast={showToast} isMobile={isMobile} />}
        {tab === "export" && <TabExport data={data} actions={actions} template={effectiveTemplate} showToast={showToast} isMobile={isMobile} isAdmin={isAdmin} />}
        {tab === "cas" && <TabCas data={data} actions={actions} showToast={showToast} isMobile={isMobile} isAdmin={isAdmin} />}
        {tab === "zahr" && <TabZahranicni data={data} actions={actions} showToast={showToast} isMobile={isMobile} isAdmin={isAdmin} zahrTemplate={zahrTemplate} />}
        {tab === "projekty" && isAdmin && <TabProjekty data={data} actions={actions} showToast={showToast} />}
        {tab === "lide" && isAdmin && <TabLide data={data} actions={actions} showToast={showToast} />}
        {tab === "sprava" && isAdmin && <TabSprava users={users} people={people} actions={actions} showToast={showToast} myEmail={(authUser.email || "").toLowerCase()} />}
      </main>
      ...
      {isMobile && (
        ...
        >
          {tabs.map((t) => (
            ...
            >
              <span style={{ fontSize: 20, filter: tab === t.id ? "none" : "grayscale(1) opacity(0.7)" }}>{t.icon}</span>
...
function TabSprava({ users, people, actions, showToast, myEmail }) {
  ...
  const addUser = async () => {
    ...
    const ok = await actions.setUser(e, { role, personIds: [] }, "Účet přidán. Nezapomeňte mu založit přihlášení ve Firebase konzoli.");
    if (ok) setEmail("");
  };
  ...
  const changeRole = (u, newRole) => {
    ...
    actions.setUser(u.id, { role: newRole, personIds: u.personIds || [] }, "Role změněna.");
  };
  ...
  const toggleTeamMember = (u, personId) => {
    ...
    actions.setUser(u.id, { role: u.role, personIds: next });
  };
      ...
      <section style={S.panel}>
        ...
        <div style={{ display: "grid", gap: 10 }}>
          ...
          <select value={role} onChange={(e) => setRole(e.target.value)} style={S.input}>
            <option value="vedouci">Vedoucí — vidí jen svůj tým (Výkazy + Export)</option>
        ...
        <div style={{ display: "grid", gap: 10 }}>
          {sortedUsers.map((u) => (
            <div key={u.id} style={{ border: `1px solid ${P.line}`, borderRadius: 10, padding: 12 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap", marginBottom: u.role === "vedouci" ? 10 : 0 }}>
              ...
              {u.role === "vedouci" && (
                <>
                  <label style={S.label}>Tým — klikáním přiřaďte podřízené ({(u.personIds || []).length})</label>
                  {people.length === 0 ? (
